...
- TCP connects succeed instantly on every port (even ones that must be closed) but the SSH banner never arrives (`Connection reset` or `timed out during banner exchange`), or `Permission denied` fires at CONNECT stage on the tailnet path: suspect a local transparent VPN, not the server. Prove it with one probe: connect to a definitely-closed port on an unrelated host (e.g. `1.1.1.1:12345`) — a completed handshake means a local middlebox is faking accepts. Inventory `Get-NetAdapter` + processes (match `ProtonVPN`, `ProTUN`, `vpn`, `warp`, `zerotier`): Proton VPN's transparent-proxy/kill-switch mode fakes handshakes, rotates the egress IP through exit nodes, and swallows Tailscale's 100.x CGNAT traffic (WSAEACCES/"General failure"). The fix is Disconnect in the VPN app — an operator step, not console or sshd work.

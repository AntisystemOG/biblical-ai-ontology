...
| weather-paper-trader | 8 PM daily | Active | Paper book + grading |
| kalshi-longshot-tracker | Nightly | Active | Paper lotto candidates |
...
| sep4-exit-watch | Hourly 11:00-19:00 CT, Sep 4 only | Active | Ridge-bet exit strategy (Thad Sep 3): peak-window dead/alive math per city (DEN 13-15, CHI/NY 14-16, MIA 13-15 CT); salvage sells + resting-order cancels pre-authorized; agent `agents/sep4-exit-watch.md`; script `scripts/kalshi_sep4_exit.py` |
| sure-thing-paper | Daily 7:50 PM CT | Active (Sep 3) | Paper ladder testing the 95 pct-odds sure-thing class nightly (BANDS ONLY - T-markets ambiguous); feeds history.json for 8 PM grading; cron `sure-thing-paper`; script `scripts/sure_thing_paper.py` |
| model-learn | Daily 8:45 PM CT | Active (Sep 3) | Settlement + learning pipeline: settles paper + live bets into the DB with WHY on losses, updates per-city NWS bias (model_state city_bias_*), reports accuracy vs 80 pct target; script `scripts/kalshi_learn.py` |
| peak-exit-watch | Half-hourly 13:00-16:30 CT daily | Active (Sep 5) | Peak-exit v2: trajectory salvage (obs_max + remaining climb vs win range) + confidence-priced exits (TWC rounding risk); replaces expired sep4-exit-watch; agent `agents/peak-exit-watch.md`; script `scripts/kalshi_peak_exit.py` |

...
```
hermes dashboard [--no-open] [--skip-build] [--port PORT] [--host HOST]
                            Start the web dashboard (requires built frontend)
...
| Ollama (local) | None / env | `OLLAMA_BASE_URL` (default `http://localhost:11434`) |
| Ollama Pro / cloud | Token | `OLLAMA_CLOUD_API_KEY` or `OLLAMA_API_KEY`; see `references/ollama-pro-cloud-setup.md` |
...
**Opening browser from WSL to Windows host.** Dashboard runs on `127.0.0.1:9119`
inside WSL but the browser lives on Windows. Use Windows-side launchers from bash:
- `cmd.exe /c start "http://127.0.0.1:9119"`
- `powershell.exe -Command "Start-Process 'http://127.0.0.1:9119'"`
Both work from an interactive WSL bash session.
...
4. **Ollama local empty** — `curl http://localhost:11434/api/tags` returns `{"models":[]}`. Either `ollama pull <model>` or switch to a cloud provider.
5. **Ollama Pro / cloud misconfiguration** — `base_url` in `config.yaml` and `auth.json` must both point to `https://api.ollama.com`; model ID must be plain (no `:cloud` suffix). The config can silently revert to localhost defaults after `hermes setup` or onboarding. Always verify three locations: `model.base_url`, `providers.ollama-launch.api`, and `auth.json` credential pool `base_url`. See `references/ollama-pro-cloud-setup.md`.
...
Hermes includes a web UI (React 19 + Vite + Tailwind CSS v4 + shadcn/ui-style components) served by a FastAPI backend. The dashboard provides a visual interface for managing the agent when running headless or alongside the CLI.

**Third-party alternatives exist — see `references/web-ui-alternatives.md` for the popular `nesquena/hermes-webui` fork (7.8k stars, no build step, three-panel chat interface).**

### Installation

Clone and build the dashboard assets (or install from PyPI where available):

```bash
# Clone dashboard source (includes React components + FastAPI backend)
git clone https://github.com/NousResearch/hermes-agent.git /path/to/hermes-agent-ui
cd /path/to/hermes-agent-ui

# Install Node dependencies (use --legacy-peer-deps to resolve React 19 compat)
npm install --legacy-peer-deps

# Build frontend assets (outputs to hermes_cli/web_dist/)
npm run build
```

Installation in editable mode (recommended if using a clone):
```bash
uv pip install -e . --python /path/to/venv/bin/python
# or: pip install -e .
```
After installing, `hermes --version` should report the cloned version.

### Usage

```bash
# Start with no auto-open, skip redundant rebuild (frontend already built)
hermes dashboard --no-open --skip-build
# Runs on 127.0.0.1:9119 by default
...
|------|--------------|-------------|
| `/` | No | Serves the built React UI |
| `/api/status` | No | Version, config version, gateway status |
| `/api/config` | Yes (session token) | Full config editor |
| `/api/health` | Yes (session token) | Health check |

When first opening `/api/config` or `/api/health` without auth, the response will tell you how to generate a session token (typically via browser dev tools after logging into the dashboard).

### Quick Switch: CLI ↔ Dashboard

Both share the same memory, skills, and backend. Pick per task:

| Situation | Reach for |
|-----------|-----------|
| Quick one-liner | `hermes chat -q "..."` |
| Long coding session | CLI (no alt-tab, keyboard-driven) |
| Point-and-click config | Dashboard (`/api/config` editor) |
| Headless / cron / scripting | CLI only |
| Showing someone else | Dashboard |

Switch mid-session:
- **CLI → dashboard:** `hermes dashboard --skip-build --no-open` then open `http://127.0.0.1:9119`
- **Dashboard → CLI:** kill the dashboard process, then `hermes` (or `hermes chat -q ...` for a single shot)

### Auto-open browser from WSL to Windows host

When the dashboard runs inside WSL but you want it to pop in the Windows-side default browser:

```bash
# cmd.exe wrapper
 cmd.exe /c start "http://127.0.0.1:9119"

# PowerShell wrapper (works when cmd.exe is missing)
 powershell.exe -Command "Start-Process 'http://127.0.0.1:9119'"
```

### Production / Persistent Run

For a persistent background instance:
```bash
nohup hermes dashboard --no-open --skip-build > /tmp/hermes-dashboard.log 2>&1 &
```
Or integrate with systemd / cronjob tool for auto-start on boot.

### Troubleshooting

**"Project: unknown" / old version reported after clone**: The editable install may still point to the original PyPI package. Re-install with the explicit venv path to pin the clone:
```bash
uv pip install -e . --python /path/to/your/venv/bin/python
hermes --version  # should show the cloned project path
```

**Frontend missing / 404 on `/`**: The `npm run build` step is required. The backend serves static files from `hermes_cli/web_dist/`; if this directory is absent, the UI won't load.

### Alternative: Nesquena WebUI

A third-party alternative dashboard (Python + vanilla JS, no build step) is documented in `references/nesquena-webui-setup.md` — covers install, "Spock" hardcoding patches, and systemd auto-start.
